...
namespace HPE
{
    public partial class FreeSpaceFinderDialog : Form
    {
        ...
        public FreeSpaceFinderDialog(ROM rom)
        {
            InitializeComponent();

            romFilePath = rom.File;
            txtNeeded.Value = 0;
            bOK.Visible = false;
            bCancel.Text = "Done";
        }

        public FreeSpaceFinderDialog(ROM rom, uint neededBytes, bool allowCancel = true)
        {
            InitializeComponent();
            ...
            if (!allowCancel)
            {
                ...
                bOK.Location = new Point(134, 282);
            }
        }
        ...
        private void FreeSpaceFinderDialog_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!canCancel && !success) e.Cancel = true;
        }
