...
namespace HPE
{
    public partial class MainForm : Form
    {
        ...
        // in-between
        private Dictionary<ushort, string> evolutionTypes = new Dictionary<ushort, string>();
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (GBABinaryReader br = new GBABinaryReader(File.OpenRead(openDialog.FileName)))
            {
                ...
                // get ini
                Ini main = new Ini(Path.Combine(Environment.CurrentDirectory, "Data", "ROMs.ini"));
                string iniPath = Path.Combine(Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file) + ".hpe.ini");
                if (!File.Exists(iniPath))
                {
                    ini = new Ini(); // new ini
                    var text = new Ini(Path.Combine(Environment.CurrentDirectory, "Data", "Text.ini"));
                    ...
                    text.CopySectionTo("Evolutions", ini);
        ...
        private void editPokédexToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (rom.File == string.Empty)
                return;

            // show editor
            using (var dlg = new PokedexDialog(rom, ini, pokemonNames))
            {
                dlg.ShowDialog();
            }
        ...
        private void PreLoadROM(GBABinaryReader br)
        {
            ...
            evolutionTypes.Clear(); cEvolutionType.Items.Clear();
            for (int b = 0; b <= 255; b++)
            {
                string evo = ini["Evolutions", b.ToString()];
                if (evo != string.Empty)
                {
                    evolutionTypes[(byte)b] = evo;
                    cEvolutionType.Items.Add(evo);
                }
                else
                {
                    cEvolutionType.Items.Add(b.ToString("X2"));
                }
            }
        ...
        private ListViewItem EvolutionToListViewItem(int id)
        {
            ...
            {
                if (evolutionTypes.ContainsKey(e.Type))
                    item.SubItems.Add(evolutionTypes[e.Type]);
                else
                    item.SubItems.Add(e.Type.ToString("X2"));
